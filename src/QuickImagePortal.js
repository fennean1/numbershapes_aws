...
export default function QuickImagePortal(props) {
  ...
  const [imgContainerStyle, setImgContainerStyle] = React.useState({
    height: "93%",
    display: "flex",
    flexDirection: "row",
  ...
  const toolLink = !data.NO_TOOL && (
    <Link target="_blank" to={{ pathname: data.TOOL }}>
      <a className="btn orange left">
  ...
  return (
    <div style={{ height: "100vh", flexDirection: "column" }}>
      ...
      >
        <div
          className="flow-text"
          style={{ margin: 10, height: window.innerHeight / 3 }}
        >
      ...
      <div style={imgContainerStyle}>
        <img
          className="boxShadow"
          style={imgStyle}
          src={require("./assets/" + data.previewImg)}
        />
      </div>
      <div style={{ margin: 10 }}>
        <Fab
