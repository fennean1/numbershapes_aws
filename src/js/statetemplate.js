...
export const init = (app, setup) => {
  ...
  function sliderAPointerUp() {
    ...
    blockRowA.value = numberline.getNumberLineFloatValueFromPosition(
      zero + blockRowA.blockWidth * blockRowA.n
    );
  ...
  function sliderBPointerUp() {
    ...
    let blockVal = numberline.getNumberLineFloatValueFromPosition(
      roundedPosition
    );
  ...
  function load(state) {
    ...
    numberline.onUpdate = () => {
      let zeroDistance = numberline.getDistanceFromZeroFromValue(
        blockRowA.value
      );
      let blockEndPoint = numberline.getNumberLinePositionFromFloatValue(
        blockRowA.value
      );
    ...
    for (let i = 0; i < 1; i++) {
      ...
      strip.x = 0;
      strip.y = (WINDOW_HEIGHT / 20) * 1.1 * j;
      strip.onUpdate = () => {
        drawWhiskers();
      };
      //strip.alpha = 0.75
